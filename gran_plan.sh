#!/bin/bash

#SBATCH --job-name=gran_plan                # Submit a job named "example"
#SBATCH --nodes=1                             # Using 1 node
#SBATCH --gres=gpu:1                         # Using 1 gpu
#SBATCH --time=0-48:00:00                     # 1 hour timelimit
#SBATCH --mem=40G                         # Using 10GB CPU Memory
#SBATCH --partition=laal_3090                        # Using "b" partition 
#SBATCH --cpus-per-task=4                     # Using 4 maximum processor

source ${HOME}/.bashrc
source ${HOME}/anaconda3/bin/activate
conda activate dino_wm
export DATASET_DIR=/home/shared/robotics
export HYDRA_FULL_ERROR=1
export EGL_GPU=$SLURM_JOB_GRES
python plan.py model_name=2025-02-04/13-38-38 n_evals=5 planner=cem goal_H=5 goal_source='random_state' planner.opt_steps=30 seed=1